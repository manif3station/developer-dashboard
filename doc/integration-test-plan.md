...
- `install.sh` prints a full progress board before it changes the system, updates that board as each bootstrap stage completes, and explains any upcoming `sudo` prompt as an operating-system package-manager password request before the prompt appears
