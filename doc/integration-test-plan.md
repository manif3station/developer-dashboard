...
- `dashboard ticket` tmux sessions move prompt indicators into a session-local two-line tmux status, keep the inline prompt free of duplicated indicators, refresh the live indicator strip automatically through `dashboard ps1 --mode tmux-status`, and do not change ordinary tmux sessions or any user tmux config file
