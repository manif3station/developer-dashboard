...
- streamed `install.sh` runs such as `curl ... | sh` succeed without a local checkout by falling back to embedded `aptfile` and `brewfile` manifests, and Debian-family hosts bootstrap `App::perlbrew` automatically when the package manager path did not already provide `perlbrew`
