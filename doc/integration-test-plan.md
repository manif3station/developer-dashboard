...
12. Browser-check the seeded `sql-dashboard` page from that fake project and confirm the profile tabs, workspace tab, schema explorer, shareable URL state, any `./.developer-dashboard/config/sql-dashboard/*.json` profiles loaded on startup, the `config/sql-dashboard` directory tightened to `0700`, and saved profile files tightened to `0600`.
