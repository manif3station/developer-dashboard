...
- explicit `dashboard skills install <source>` runs append the exact source to the home root `~/.developer-dashboard/ddfile` without duplicating existing non-comment entries, and bare `dashboard skills install` uses that root `ddfile` to reinstall every registered skill as an update-all pass
