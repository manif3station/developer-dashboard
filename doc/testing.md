...
- browser-check the seeded `sql-dashboard` bookmark in a fresh runtime and verify the profile tabs, merged `SQL Workspace` tab, workspace left-nav collection tabs plus saved SQL list, visible active saved-SQL label, schema explorer, shareable `connection` URL state, file-backed connection profiles loaded from `config/sql-dashboard`, file-backed SQL collections loaded from `config/sql-dashboard/collections`, both directory trees at `0700`, and saved profile/collection files at `0600`
- when changing the bookmark-local SQL execution, collection, or schema browser flow, run `prove -lv t/27-sql-dashboard-playwright.t` to keep driver-dropdown DSN rewriting, profile editing, multi-SQL collection saving, workspace navigation layout, active saved-SQL labeling, shared-URL draft-profile restoration, schema-table tabs, and owner-only sql-dashboard persistence browser-verified
