...
- when changing the bookmark-local SQL execution, collection, or schema browser flow, run `prove -lv t/27-sql-dashboard-playwright.t` for the fake-driver browser route, `PERL5LIB=/tmp/sql-lib/lib/perl5:/tmp/sql-lib/lib/perl5/x86_64-linux-gnu-thread-multi prove -lv t/31-sql-dashboard-sqlite-playwright.t` for the 50-case real SQLite browser matrix, and `PERL5LIB=/tmp/sql-lib/lib/perl5:/tmp/sql-lib/lib/perl5/x86_64-linux-gnu-thread-multi prove -lv t/32-sql-dashboard-rdbms-playwright.t` when `DBD::mysql` and `DBD::Pg` are installed so optional docker-backed MySQL/PostgreSQL coverage stays healthy too
