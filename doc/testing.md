...
- browser-check the seeded `sql-dashboard` bookmark in a fresh runtime and verify the profile tabs, merged `SQL Workspace` tab, workspace left-nav collection tabs plus saved SQL list, visible active saved-SQL label, large auto-resizing editor, quiet action row beneath the editor, inline `[X]` delete affordances in the saved-SQL list, schema explorer, shareable `connection` URL state, file-backed connection profiles loaded from `config/sql-dashboard`, file-backed SQL collections loaded from `config/sql-dashboard/collections`, both directory trees at `0700`, and saved profile/collection files at `0600`
...
- when changing the bookmark-local SQL execution, collection, or schema browser flow, run `prove -lv t/27-sql-dashboard-playwright.t` for the fake-driver browser route, `PERL5LIB=/tmp/sql-lib/lib/perl5:/tmp/sql-lib/lib/perl5/x86_64-linux-gnu-thread-multi prove -lv t/31-sql-dashboard-sqlite-playwright.t` for the 51-case real SQLite browser matrix, and `PERL5LIB=/tmp/sql-lib/lib/perl5:/tmp/sql-lib/lib/perl5/x86_64-linux-gnu-thread-multi prove -lv t/32-sql-dashboard-rdbms-playwright.t` when `DBD::mysql`, `DBD::Pg`, `DBD::ODBC`, and `DBD::Oracle` are installed so optional docker-backed MySQL/PostgreSQL/MSSQL/Oracle coverage stays healthy too; on this host MSSQL and Oracle also expect the user-space client libraries to be exposed through `LD_LIBRARY_PATH` and, for Oracle, `ORACLE_HOME`
