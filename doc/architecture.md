...
- the seeded `sql-dashboard` bookmark is also built entirely inside the bookmark runtime, with file-backed connection profiles under `config/sql-dashboard/<profile-name>.json`, owner-only `config/sql-dashboard` directory/file permissions (`0700` / `0600`), tabbed profile and schema-table navigation, browser URL restoration for active profile/table/sql state instead of saved SQL files, programmable `SQLS_SEP` / `INSTRUCTION_SEP` statement hooks, generic `DBI` execution, and schema browsing through `table_info` / `column_info`
