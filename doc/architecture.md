...
- the public `dashboard` entrypoint stays thin for lightweight commands such as `jq`, `yq`, `of`, `open-file`, `ticket`, `path`, `paths`, `ps1`, and `version`, and the shipped starter bookmark source plus helper script source now live under `share/seeded-pages/` and `share/private-cli/` so those bodies do not bloat the command script; installed copies resolve the same assets from the distribution share dir
