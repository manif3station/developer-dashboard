...
- saved bookmark `Ajax` helper calls can avoid transient tokens by supplying `file => 'name.json'`, which stores the code under the runtime cache, emits `/ajax?page=<bookmark>&file=<name>&type=...`, and runs the stored file as a real process so live `stdout` and `stderr` stream back to the browser directly
