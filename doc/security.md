...
must depend on the build job, and the provenance job's steps must verify the
checksum and must not invoke `prove`, `dzil`, or `cpanm`. Those assertions exist
so a later change cannot quietly collapse the two jobs back into one.
