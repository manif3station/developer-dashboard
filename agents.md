...
* `FULL-POD-DOC` means comprehensive documentation for every single repo-owned
  Perl module and Perl script.
* The documentation must be specific to that exact file, not a boilerplate
  block with the filename swapped.
* The contributor-definition docs must also keep at least 10 common-path
  example patterns and 10 edge/debugging example patterns with explanation, so
  future rewrites have a concrete quality reference.
* Every Perl file must explain:
  * what the file owns
  * the real inputs it accepts
  * the real outputs or side effects it produces
  * how it fits into the runtime or command flow
  * when contributors should edit it instead of another file
  * multiple concrete examples that match real usage, including the common
    path plus at least one meaningful edge or debugging path when the file
    owns one
* One-line POD, placeholder prose, or generic repeated wording does not satisfy
  `FULL-POD-DOC`.
* The contributor contract belongs in contributor-facing docs such as this file,
  `AGENTS.override.md`, and `doc/testing.md`, not in the top-level product
  manual in `README.md` or `Developer::Dashboard.pm`.
...
* When a required closeout step is still missing, do not stop to ask whether to continue with that step.
* If tests, coverage, packaging, commit, push, or other required flow gates are still outstanding, complete them automatically unless the user explicitly says to stop.
* Do not use soft handoff questions such as `If you want, ...` once a mandatory gate miss has already been identified.
...
20. **Meaningful git commits only**
...
23. **DO NOT DO PAUSE release rulesi until User ask you to**

* DO NOT Release locally to PAUSE only when doing an actual release until the uaser ask you to
