...
- Fixed docker compose CLI passthrough parsing so real docker compose flags such as `-d` and `--build` are no longer rejected as dashboard wrapper options before the final command is executed.
