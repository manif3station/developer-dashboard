...
1. If `scorecard` is installed and the repository is accessible, run it and record the result.
2. If `scorecard` is not installed or cannot inspect the active repository, say so explicitly and do not claim Scorecard passed.
3. Treat any of the following as stop-and-fix issues:
...
4. For intentionally user-driven tools such as `sql-dashboard`, raw SQL execution must stay explicit and user-authored.
5. Do not add hidden query concatenation.
6. Do not add background SQL generation around user-driven SQL tools.
...
54. Do not perform a PAUSE release unless the user explicitly asks for it.
55. If a PAUSE release is requested, tag `PAUSE_RELEASED_HERE` after release.

## 7. User config safety

56. `dashboard init` must not overwrite `config/config.json`.
57. Existing config must remain intact on repeated init runs.
