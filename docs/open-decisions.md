...
| 27 | 2026-08-09 | **DD-516 — two defects in the card reminder that this project is not allowed to fix on its own, and neither is blocking you.** (1) `Tira::project_new` creates the protected `discard` column with no `watched` key, and `column_list` resolves an absent key to watched=1, so **every newly created Tira board chases its own discarded cards forever**. It is the worst case rather than a marginal one because a card that has never moved is unmeasured and never chased, while *discarding is itself a move* — the one action that ends a card's life starts its reminder clock. Reproduced on a throwaway board: the discarded card was chased, the untouched backlog card was not. (2) `_dwell_start` counts only moves, so **a comment does not reset a card's clock** — the reminder's own advice, "leave a comment saying what it is waiting for", reads as activity to a person and as nothing at all to the reminder; a card under active discussion is still reported as stalled, and a reminder cannot be answered in place. This board is already safe from (1) — `watched: false` is set explicitly on all three board types and `.claude/tools/reminder-column-guard` asserts it hourly — so nothing here waits on you; the question is only whether the next board anyone creates inherits it, and whether (2) should change. | Open — **not blocking**; DD-516 sits in `blocked-by-michael` | Both fixes are in `~/.developer-dashboard/skills/tira/lib/Tira.pm`, a single installed copy shared with the collector serving `/home/mv/.tira/mt5`. Your standing rule is that shared skills are read-only and each project keeps its own copy, so I did not touch it. Three options: **(a) fix it upstream in the skill** — small, and both boards benefit, but it changes mt5's reminders without mt5 asking; **(b) fork a project-local copy of the skill** — sanctioned by the rule, mt5 untouched, but a whole fork that will drift, for two small fixes; **(c) leave both** — this board is correct today and the sharp edges are documented in `docs/gate-map.md`. Recommendation: **(a)**, since for mt5 it is strictly quieter — but it is your system, so it is your call. Three rounds filed this independently as DD-513, DD-514 and DD-516; the first two are discarded and DD-516 is the survivor |
| 28 | 2026-08-09 | **DD-519 — the card reminder starts a new agent per reminder, and they share one checkout.** Six `claude -p --resume` processes were running against this repository at once, spawned about a minute apart for DD-510 — a card that was correctly parked and could not move. They fight over one working tree: `t/108` reported 3 failures, then 10, then 3 again from an unchanged tree because concurrent runs create and delete the same fixture directories in the repository root; `t/108` and `.claude/tools/reminder-column-guard` were each modified by another process mid-edit; and DD-517's gate fix was committed by a different process between this one's `git add` and its `git commit`. The reminder exists so work is not forgotten, but as built it can corrupt the work it is reminding about, and it does so hardest for cards that are legitimately stuck. | Open — **needs your call**; DD-519 sits in `blocked-by-michael` | Mitigated but not fixed: `blocked-by-dependency` is now unwatched, which stopped this particular flood, because a card blocked by another card is not actionable — the action is on the blocker, and the blocker is watched in its own right. That narrows the trigger without changing the spawning. The fix is in whatever launches the agent, which is outside this repository (`~/dd-tg`), so I have not touched it. Options: **(a)** refuse to start a second agent for a card that already has one; **(b)** a per-checkout lock so the second process waits or exits rather than sharing the tree — `~/dd-tg/dd-round-lock.sh` already implements this pattern for rounds and could cover reminder-spawned agents too; **(c)** leave it and rely on unwatching columns to keep the volume down. Recommendation: **(b)**, since the lock already exists and the failure it prevents is silent data corruption rather than noise |

---

## SUPERSEDED as of 2026-08-09 — questions now live on the cards

The owner has asked that questions stop being kept in this file and be raised on
the card they concern, using Tira's own question commands:

    d2 tira.question.ask    --ref DD-NNN --text "..." --author "..."
    d2 tira.question.list   --ref DD-NNN            # reading marks the answers read
    d2 tira.question.mark   --id Q-NNN --mark ok|not-ok
    d2 tira.question.answer --id Q-NNN --text "..."

A question reference is project-wide, so `d2 tira.search --text Q-007` finds the
card it was asked on whichever board that is. A `not-ok` mark settles nothing by
itself — ask a new question as well, because there are no follow-up threads.

Nothing above is deleted: this page stays as the record of what was asked and
answered before the change. The two questions that were still open here have been
moved onto their cards and are answered there from now on:

- row 24, the operator repository with no remote, is now **Q-001** on DD-504
- row 25, where git-gate-complete but unpublished work belongs, is now **Q-002** on DD-511

and the rebuild question raised since is **Q-003** on DD-522.
