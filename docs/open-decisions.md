...
| 24 | 2026-08-09 | DD-504 found that `~/dd-tg` has no git remote, so seven finished cards — **DD-455, DD-490, DD-494, DD-496, DD-497, DD-500, DD-501** — are committed to exactly one disk and were never pushed. Where should the operator repository push to — a private GitHub repo, somewhere else, or is one-disk deliberate? | Open — **not blocking**, proceeding on the assumption that one disk is deliberate until told otherwise | Raised after you answered the others, so it was not on the page when you did |
