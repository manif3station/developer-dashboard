...
is( $version, '2.03', 'repo version bumped for the Scorecard gatekeeper hardening release' );
like( $pm, qr/^2\.03$/m, 'main POD version matches the module version' );
if ( $dist ne '' ) {
    like( $dist, qr/^version = 2\.03$/m, 'dist.ini version matches the module version in the source tree' );
    ...
    like( $meta, qr/"version"\s*:\s*"2\.03"/, 'META.json version matches the module version in the built distribution' );
}
like( $changes, qr/^2\.03\s+2026-04-08$/m, 'Changes top entry matches the bumped version' );
