...
is( $version, '1.89', 'repo version bumped for the canonical DD-OOP-LAYERS path fix release' );
like( $pm, qr/^1\.89$/m, 'main POD version matches the module version' );
if ( $dist ne '' ) {
    like( $dist, qr/^version = 1\.89$/m, 'dist.ini version matches the module version in the source tree' );
    ...
    like( $meta, qr/"version"\s*:\s*"1\.89"/, 'META.json version matches the module version in the built distribution' );
}
like( $changes, qr/^1\.89\s+2026-04-07$/m, 'Changes top entry matches the bumped version' );
