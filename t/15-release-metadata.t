...
is( $version, '2.11', 'repo version bumped for the RESULT overflow and legacy seed refresh fixes' );
like( $pm, qr/^2\.11$/m, 'main POD version matches the module version' );
if ( $dist ne '' ) {
    like( $dist, qr/^version = 2\.11$/m, 'dist.ini version matches the module version in the source tree' );
    ...
    like( $meta, qr/"version"\s*:\s*"2\.11"/, 'META.json version matches the module version in the built distribution' );
}
like( $changes, qr/^2\.11\s+2026-04-09$/m, 'Changes top entry matches the bumped version' );
