...
is( $version, '2.20', 'repo version bumped for the hook stop and LAST_RESULT release' );
like( $pm, qr/^2\.20$/m, 'main POD version matches the module version' );
if ( $dist ne '' ) {
    like( $dist, qr/^version = 2\.20$/m, 'dist.ini version matches the module version in the source tree' );
    ...
    like( $meta, qr/"version"\s*:\s*"2\.20"/, 'META.json version matches the module version in the built distribution' );
}
like( $changes, qr/^2\.20\s+2026-04-10$/m, 'Changes top entry matches the bumped version' );
