...
is( $version, '2.10', 'repo version bumped for the GitHub workflow release-hardening update' );
like( $pm, qr/^2\.10$/m, 'main POD version matches the module version' );
if ( $dist ne '' ) {
    like( $dist, qr/^version = 2\.10$/m, 'dist.ini version matches the module version in the source tree' );
    ...
    like( $meta, qr/"version"\s*:\s*"2\.10"/, 'META.json version matches the module version in the built distribution' );
}
like( $changes, qr/^2\.10\s+2026-04-09$/m, 'Changes top entry matches the bumped version' );
